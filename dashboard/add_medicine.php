...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO medicines (name, description, description1, description2, dosage, dosage1, dosage2, type_id, other_names, uses, uses1, uses2, type_name) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $name, $description, $description1, $description2, $dosage, $dosage1, $dosage2, $type_id, $other_names, $uses, $uses1, $uses2, $type_name
            ]);
...
<head>
    ...
    <title>Add Medicine - Medicine Management System</title>
    <!-- Bootstrap CSS -->
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    <h1 class="mb-3 mb-md-0"><i class="fas fa-plus-circle text-primary me-2"></i>Add New Medicine</h1>
